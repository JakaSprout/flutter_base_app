...
class _BannerSectionState extends State<BannerSection> {
  ...
  void _onScroll() {
    ...
    final cardsLength = HomeConstants.harvestCalculatorCards.length;

    if (newPage != _currentPage && newPage >= 0 && newPage < cardsLength) {
      setState(() {
      ...
      children: [
        ...
        SizedBox(
          ...
          child: ListView.builder(
            ...
            padding: const EdgeInsets.only(right: _peekWidth),
            itemCount: cards.length,
            itemBuilder: (context, index) {
              return Padding(
                padding: EdgeInsets.only(
                  right: index < cards.length - 1 ? _cardSpacing : 0,
                ),
                child: SizedBox(
                  width: _cardWidth,
                  child: BannerCard(
                    card: cards[index],
                    onTap: () => widget.onCardTap?.call(cards[index].id),
                  ),
                ),
              );
            },
...
class BannerCard extends StatelessWidget {
  ...
  const BannerCard({required this.card, super.key, this.onTap});

  /// Card data
  final HarvestCalculatorCardData card;
