...
void main() {
  group('GraphScreen', () {
    ...
    testWidgets('should have correct structure', (WidgetTester tester) async {
      ...
      // Assert
      // TestHelpers.createTestApp wraps with Scaffold, so GraphScreen's Scaffold is nested
      expect(find.byType(Scaffold), findsWidgets); // At least 2 Scaffolds (wrapper + screen)
      expect(find.byType(Center), findsWidgets); // At least 1 Center (from GraphScreen)
      expect(find.byType(Column), findsOneWidget); // Column from GraphScreen
